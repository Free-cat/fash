...
COPY = Copy(
    ...
    photo_ready=(
        ...
        "У тебя *{free_credits} бесплатные примерки*.\n\n"
        "👉 *Отправь фото одежды* — платье, куртку, что угодно — и увидь себя в образе."
    ),
    ...
    btn_style_guide_showcase="✨ Полный стайлинг — 1",
    ...
    premium_showcase_offer_v1=(
        "Твой первый полный стайлинг — 9 образов, палитра и аксессуары "
        "к этой вещи. *Всего 1 примерка.*"
    ),
    premium_showcase_offer_v2=(
        "Попробуй полный стайлинг один раз — 9 образов с этой вещью, "
        "твоя палитра и аксессуары. *Сегодня за 1 примерку.*"
    ),
    premium_offer_preview_caption="Вот что получаешь — твоя версия будет с этим образом",
    premium_offer_cross_sell=(
        "Для полного стайлинга нужно {cost} примерки, у тебя {balance}. "
    ...
    premium_showcase_failed=(
        "Не получилось собрать стайлинг — вернул(а) примерку. Попробуем ещё раз?"
    ),
    ...
    balance_text=(
        "У тебя *{balance} примерок*.\n\n"
        "Докупи сейчас — в больших пакетах дешевле за образ 👇"
    ),
    balance_empty=(
        "Примерки кончились 👀\n\n"
        "Возьми пакет и продолжай — в больших пакетах дешевле за образ.\n"
        "Или позови друга — *+1 бесплатно* ✨"
    ),
    balance_line="1 примерка = 1 образ на тебе.",
